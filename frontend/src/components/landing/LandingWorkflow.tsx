...
export function LandingWorkflow() {
  return (
    <section id="workflow" className="mx-auto max-w-7xl px-4 py-20 sm:px-6 md:py-24 lg:px-8">
      <SectionHeading
        ...
        <ol className="grid gap-4 lg:grid-cols-5">
          {workflowSteps.map((step) => (
            <li key={step.eyebrow} className="relative">
              <Card className="h-full rounded-[28px] border-slate-200/80 bg-white/90 p-6 shadow-[0_18px_50px_-34px_rgba(15,23,42,0.24)]">
                <span className="relative z-10 flex size-12 items-center justify-center rounded-2xl border border-sky-200 bg-sky-50 text-sm font-semibold text-sky-700 shadow-sm">
                  {step.eyebrow}
                </span>
                <p className="mt-5 text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-400">
                  {step.highlight}
                </p>
                <h3 className="font-heading mt-3 text-xl font-semibold tracking-tight text-slate-950">
                  {step.title}
                </h3>
      ...
      <div className="mt-6 flex items-start gap-3 rounded-[24px] border border-slate-200/80 bg-slate-50/90 px-5 py-4 text-sm leading-6 text-slate-600 shadow-sm">
